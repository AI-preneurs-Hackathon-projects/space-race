...
test('short real-time flights at baseline and stage 25 accept steering and fire under pressure',async({browser})=>{
 ...
 for(const stage of [1,25]){
  ...
  await page.route('**/components/cargo-game.tsx*',async route=>{const response=await route.fetch(),body=await response.text();expect(body.split('newCampaign(1979)')).toHaveLength(2);await route.fulfill({response,body:body.replace('newCampaign(1979)',`({...newCampaign(1979),stage:${stage}})`)});});
  ...
  expect(seen.size).toBeGreaterThan(0);expect(lastProgress).toBeGreaterThan(15);expect(xMax-xMin).toBeGreaterThan(1);expect(errors).toEqual([]);await expect(page.getByText('Shipment lost.',{exact:true})).not.toBeVisible();await page.close();
 }
