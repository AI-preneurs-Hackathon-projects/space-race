...
for(const fallback of [false,true])test(`contract transfer loads a fresh manifest, repairs hull at gate, and requests a ship-aware section plan${fallback?' with fallback':''}`,async({page})=>{
 ...
 await page.route('**/api/director',r=>r.fulfill({status:503,json:{error:'Local contract test'}}));
 ...
 await expect(page.locator('.contract-gate')).toContainText('Delivery complete');await expect(page.locator('.delivery-ledger')).toContainText('37.2%');
