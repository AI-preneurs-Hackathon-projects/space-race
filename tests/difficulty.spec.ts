...
import {CRUISE_SPEED,FLEET,type Hull} from '../lib/game/types';
...
test('both optional portals remain reachable during faster late-stage runs',()=>{
 for(const stage of [1,9,25])for(const seed of [1,3,7]){
  ...
  for(let i=0;i<1600&&s.status==='flying';i++){
   ...
   // Evade during the long approach; parking at a distant gate invites sustained fire.
   // The final four seconds allow normal thrust to align while countering current drift.
   const align=gate&&-gate.z/(CRUISE_SPEED*Math.max(.5,s.speed))<=4;
   const action=align?{x:Math.max(-1,Math.min(1,(gate.x-s.x)*2-s.vx*.2)),y:Math.max(-1,Math.min(1,(gate.y-s.y)*2-s.vy*.2)),fire:true}:evasivePilot(s);
...
test('seeded baseline and later runs reward steering rather than idle or stationary fire',()=>{
 ...
 for(const r of results){expect(r.peak).toBeLessThan(100);expect(r.minReaction).toBeGreaterThan(1.28);expect(r.firstHit+1e-9).toBeGreaterThan(4);}
});
