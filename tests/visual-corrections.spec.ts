...
for(const width of [1224,390])test(`gate label and jump presentation at ${width}px`,async({page})=>{
 ...
 for(const z of [-300,-100,-40,-8]){
  await page.evaluate(z=>{const q=(window as any).__portalQA;Object.assign(q.state,{progress:10,x:0,y:0,shield:0,warpAge:null,entities:[],effects:[]});const e=q.spawnObject(q.state,'portal',-5,0,z);e.radius=3.25;},z);await page.waitForTimeout(200);
  ...
  if(z===-300||z===-100||z===-40){const opening=await page.evaluate(()=>{const q=(window as any).__portalQA,e=q.state.entities[0],v=q.camera.position.clone().set(e.x,e.y+e.radius*1.3,e.z).project(q.camera),c=document.querySelector('canvas')!.getBoundingClientRect();return c.top+(-v.y*.5+.5)*c.height;});expect(result.bottom).toBeLessThan(opening-5);}
 ...
 await page.evaluate(()=>{const q=(window as any).__portalQA;Object.assign(q.state,{entities:[]});q.spawnObject(q.state,'portal',-5,0,-100).radius=3.25;});await page.waitForTimeout(100);
 await page.evaluate(()=>{const q=(window as any).__portalQA,r=document.querySelector('.portal-label')!.getBoundingClientRect(),c=document.querySelector('canvas')!.getBoundingClientRect(),v=q.camera.position.clone().set(((r.left+r.width/2-c.left)/c.width)*2-1,1-((r.top+r.height/2-c.top)/c.height)*2,.5).unproject(q.camera).sub(q.camera.position).normalize(),p=q.camera.position.clone().addScaledVector(v,(-80-q.camera.position.z)/v.z);q.spawnObject(q.state,'iron-asteroid',p.x,p.y,p.z).radius=2;});await page.waitForTimeout(100);await expect(page.locator('.portal-label')).toBeHidden();
 // Protection still exists; its enclosing sphere and radial jump burst are absent during warp.
