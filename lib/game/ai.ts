...
export async function readInput(request:Request){
 ...
 if(Number(request.headers.get("content-length"))>250000)throw new AIError("The sketch is too large.",413);
 const reader=request.body?.getReader();if(!reader)throw new AIError("Missing request.",400);let total=0;const chunks:Uint8Array[]=[];
 while(true){const {done,value}=await reader.read();if(done)break;total+=value.byteLength;if(total>250000){await reader.cancel();throw new AIError("The sketch is too large.",413);}chunks.push(value);}
...
let nextRequest=0;
export async function generateJSON(name:string,schema:object,prompt:string,image?:string){
 ...
 try{response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},signal:AbortSignal.timeout(25000),body:JSON.stringify({model,store:false,max_output_tokens:2400,input:[{role:"user",content}],text:{format:{type:"json_schema",name,strict:true,schema}}})});}catch{throw new AIError("OpenAI took too long to respond. Try again, or use the local option.");}
