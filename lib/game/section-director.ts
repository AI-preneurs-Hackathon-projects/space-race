...
Launch hull integrity is ${Math.round(integrity*100)}%. A final Warp Gate repairs the hull fully; previousArrival records damage before that repair. ${cargoRules} At low cargo or weak arrival hull, provide manageable pressure and recovery space without erasing the chosen difficulty. All hostile ships belong to pirates. Easy still requires steering and favors fragile rocks, shorter enemy sequences and calm spacing; Hard should feature tougher opponents and more intense sequences while retaining fair gaps. Each later section continues the local progression. Agile ships can face more directional variety; armored slow ships need room to steer. Never assume a named ship's default stats instead of the supplied effective stats.
${contractRules}
Include rocks, at least ${Math.round((challenge.waves-2)*({easy:.32,normal:.42,hard:.5}[context.difficulty]+(context.contract?.risk==='high'?.06:context.contract?.risk==='low'?-.03:0)))} armed encounters and at least three calmer beats spread through the section. Avoid repeated fields and more than ${context.difficulty==='easy'?1:context.difficulty==='hard'?3:2} armed waves in a row. Fields are moon, blackhole and repulsor. Armed ships are pirate, twinwing-fighter, saucer-cruiser and wedge-destroyer. Fuel-tank and volatile-rock support chain reactions. Cargo-crate and repair-pod are spent solid debris and restore nothing. Shield-buoy gives temporary protection only. At most 10% of beats may be shield-buoy and at most 20% may be fields. Do not flood the route with pickups or fields. Keep variety across the opening, middle and final approach; the section has encounters from course position 8 through 146 and a final gate at 150. Return only the constrained JSON plan.`;
}
