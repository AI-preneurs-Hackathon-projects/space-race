...
export const CARGO_TYPES = ['medical_supplies', 'volatile_fuel', 'high_value_technology', 'navigation_computers', 'cryogenic_passengers'] as const;
export type CargoType = typeof CARGO_TYPES[number];
export const RISKS = ['low', 'medium', 'high'] as const;
export type Risk = typeof RISKS[number];
export const DELIVERY_URGENCIES = ['standard', 'priority', 'express'] as const;
export type DeliveryUrgency = typeof DELIVERY_URGENCIES[number];
/** Sector-active seconds, derived by local code after the loadout is final. */
export type DeliverySpeedBonus = {maxReward: number; fullBonusSeconds: number; expiresSeconds: number; extraGateLimit: number};
export const CARGO_MODIFIERS = ['fragile_cargo', 'unstable_fuel', 'valuable_cargo', 'tracked_signal', 'life_support'] as const;
export type CargoModifier = typeof CARGO_MODIFIERS[number];
export const THREAT_TYPES = ['pirates', 'asteroids', 'bounty_hunters', 'pirate_pursuers', 'cargo_hazards'] as const;
export type ThreatType = typeof THREAT_TYPES[number];
export const EVENT_TYPES = ['pirate_ambush', 'enemy_reinforcement', 'asteroid_wave', 'repair_opportunity', 'bounty_hunter', 'pirate_pursuit', 'cargo_instability', 'shortcut_available'] as const;
...
export const CARGO: Record<CargoType, CargoDefinition> = {
  ...
  navigation_computers: {label:'Prototype navigation computers', description:'Prototype navigation computers broadcast a test signal that attracts pirate pursuers. Keep moving to evade their fire.', modifiers:['tracked_signal'], minimumIntegrity:null, damageMultiplier:1, hazardMultiplier:1, instabilityPerDamage:0, targetPressure:1.15, lifeSupportDrain:0},
...
export type DirectorContext = {
  ...
  activeContract?: Pick<Contract, 'risk' | 'possibleThreats' | 'possibleObjectives' | 'minimumCargoIntegrity' | 'urgency' | 'speedBonus'>;
  availableTargets?: {shieldBuoys: number};
  effectiveCruise?: number;
  shortcutBudget?: {spawned: number; used: number; limit: number};
  deliveryTiming?: {elapsed: number; fullBonusSeconds: number; expiresSeconds: number; extraGatesRemaining: number; shortcutEligible: boolean};
  hull: number;
  maxHull: number;
