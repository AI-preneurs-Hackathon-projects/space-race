...
export function difficulty(stage:number,setting:DifficultySetting='normal'){
 ...
 const tuning={easy:{waves:30,gap:4.9,portal:3.35,motion:1,interval:1.85,aim:.25,bullet:30},normal:{waves:34,gap:4.5,portal:3.2,motion:1.15,interval:1.6,aim:.32,bullet:32},hard:{waves:36,gap:4.1,portal:3.05,motion:1.35,interval:1.28,aim:.42,bullet:36}}[setting];
 return {tier,label:tier<3?'CONTESTED':tier<7?'HOSTILE':tier<13?'EXTREME':'DEEP SPACE',waves:tuning.waves+Math.round(pressure*4),gapWidth:tuning.gap-pressure*(setting==='normal'?.65:.6),portalRadius:tuning.portal-pressure*(setting==='normal'?.35:.3),pirateMotion:tuning.motion+pressure*(setting==='easy'?.9:setting==='hard'?1.1:1.05),pirateInterval:tuning.interval-pressure*(setting==='easy'?.5:setting==='hard'?.4:.48),aimLead:tuning.aim+pressure*(setting==='easy'?.25:setting==='hard'?.28:.3),bulletSpeed:tuning.bullet+pressure*(setting==='easy'?7:setting==='hard'?9:8),volley:tier>=(setting==='easy'?5:setting==='hard'?1:2)?2:1};
}
...
export function stageMission(stage:number,seed:number,previous:{x:number;y:number}[]=[],template?:Mission,setting:DifficultySetting='normal'):Mission{
 const d=difficulty(stage,setting),risk=template?.contract?.risk,rand=random(seed),events:Encounter[]=[],portals:{x:number;y:number}[]=[];
 if(risk==='high')d.gapWidth-=.2;
 ...
 // Engine-owned combat density prevents a calm AI plan from erasing the selected challenge.
 const plannedTypes:ObjectType[]=Array.from({length:d.waves-2},(_,i)=>ai?.[i]?.objectType??roster[(i+(stage-1)*5)%roster.length]);
 const share=({easy:.32,normal:.42,hard:.5}[setting])+(risk==='high'?.06:risk==='low'?-.03:0);
 const minimumArmed=Math.round(plannedTypes.length*share),maximumArmed=Math.ceil(plannedTypes.length*(share+.06));
 let armed=plannedTypes.filter(isShip).length;
 for(let i=plannedTypes.length-1;i>=0&&armed>maximumArmed;i--)if(isShip(plannedTypes[i])){plannedTypes[i]='ice-asteroid';armed--;}
 while(armed<minimumArmed){
  let best=-1,bestSpace=-1;
  for(let i=1;i<plannedTypes.length-1;i++){
   if(isShip(plannedTypes[i]))continue;
   let left=0,right=0;for(let j=i-1;j>=0&&isShip(plannedTypes[j]);j--)left++;for(let j=i+1;j<plannedTypes.length&&isShip(plannedTypes[j]);j++)right++;
   if(left+right>=maxShipStreak)continue;
   const space=Math.min(...plannedTypes.flatMap((id,j)=>isShip(id)?[Math.abs(i-j)]:[]));
   const priority=space+(OBJECTS[plannedTypes[i]].family==='salvage'?10:OBJECTS[plannedTypes[i]].family==='rock'?0:4);
   if(priority>bestSpace){best=i;bestSpace=priority;}
  }
  if(best<0)break;
  plannedTypes[best]=setting==='hard'&&armed%3===0?'saucer-cruiser':armed%2?'twinwing-fighter':'pirate';armed++;
 }
 for(let i=0;i<d.waves;i++){
  ...
  let id=plannedTypes[objectIndex++];
  ...
  if(isShip(id)){events.push({at,arrival,kind:'pirate',objectType:id,x:sign*(3.6+rand()*1.8),y:(rand()-.5)*4,count:1,escort:i%2===0?'fuel-tank':'volatile-rock'});continue;}
  if(spec.family==='rock'){
