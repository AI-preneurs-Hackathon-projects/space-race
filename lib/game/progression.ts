import {clamp,FLEET,DURATION,type Hull,type Mission,type Encounter} from './types';
...
export function stageMission(stage:number,seed:number,previous:{x:number;y:number}[]=[],template?:Mission):Mission{
 ...
 for(let i=0;i<d.waves;i++){
  const arrival=8+i*((DURATION-12)/(d.waves-1)),at=Math.max(0,arrival-22),portalIndex=portalIndices.indexOf(i);
 ...
 return {title:`${stageEnvironment(stage).name} passage`,source:ai?'openai':'practice',note:ai?'AI plan · local stage safety':'Seeded local stage',events,stage,seed,challenge:d};
}
