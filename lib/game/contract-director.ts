...
export function fallbackContracts(context:DirectorContext,targetSector=context.sector+1):Contract[] {
  ...
  const highOptions:CargoType[]=context.notableEvents.includes('authority_scan')?['volatile_fuel','classified','high_value_technology']:context.performance.kills>=3?['classified','volatile_fuel','high_value_technology']:['volatile_fuel','classified','high_value_technology'];
...
export function validateContracts(value:unknown,context:DirectorContext,targetSector=context.sector+1):Contract[] {
  ...
  const contracts=v.contracts.map((item,index)=>{
    const c=record(item);keys(c,['id','title','description','cargoType','destination','sector','reward','risk','minimumCargoIntegrity','modifiers','possibleThreats','possibleObjectives','mystery']);
    ...
    const signatureThreat:Partial<Record<CargoType,ThreatType>>={high_value_technology:'bounty_hunters',classified:'authorities',volatile_fuel:'cargo_hazards'};
    ...
    const [min,max]=contractRewardBounds(risk,sector);
    const contract=makeContract(cargoType,risk,sector,context,finite(c.reward,min,max,true),words(c.title,60,TITLES[cargoType]));
...
const HOSTILE_EVENTS:EventType[]=['pirate_ambush','enemy_reinforcement','bounty_hunter','authority_scan'];
...
export function isEventLegal(event:EventProposal,context:DirectorContext):boolean {
  ...
  if(context.notableEvents.slice(-2).includes(event.type))return false;
  const requiredThreat:Partial<Record<EventType,ThreatType>>={pirate_ambush:'pirates',enemy_reinforcement:'pirates',asteroid_wave:'asteroids',bounty_hunter:'bounty_hunters',authority_scan:'authorities',cargo_instability:'cargo_hazards'};
  ...
  switch(event.type){
    ...
    case 'authority_scan':return context.currentCargo==='classified';
    ...
    case 'shortcut_available':return context.routeProgress>.2&&context.routeProgress<.75;
    case 'asteroid_wave':return context.cargoIntegrity>35&&context.hull/context.maxHull>.35;
    case 'pirate_ambush':return context.currentCargo==='high_value_technology'||context.currentCargo==='classified'||context.performance.kills>0||context.sector>=2;
  }
}
...
export function fallbackDecision(context:DirectorContext):DirectorDecision {
  ...
  if(context.currentCargo==='classified')candidates.push('authority_scan');
  ...
  else if(event?.type==='authority_scan')conditions=[{type:'ESCAPE_ENEMIES',target:'hostiles',successCondition:0,duration:30}];
...
export function contractDirectorSchema(mode:DirectorMode,context?:DirectorContext):object {
  ...
  if(mode==='contracts'){
    const contract=schemaObject({id:{type:'string'},title:{type:'string'},description:{type:'string'},cargoType:enumString(CARGO_TYPES),destination:{type:'string'},sector:{type:'integer'},reward:{type:'integer'},risk:enumString(RISKS),minimumCargoIntegrity:{type:['number','null']},modifiers:{type:'array',items:enumString(CARGO_MODIFIERS),minItems:1,maxItems:2},possibleThreats:{type:'array',items:enumString(THREAT_TYPES),minItems:1,maxItems:4},possibleObjectives:{type:'array',items:enumString(OBJECTIVE_TYPES),minItems:1,maxItems:5},mystery:{type:['string','null'],enum:[null,...Object.values(MYSTERIES)]}});
