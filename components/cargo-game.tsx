...
import { ArrowUpRight, ArrowRight, Box, Crosshair, Hexagon, PencilLine, Play, Pause, RotateCcw, Shield, Zap, ChevronRight, Radio, Navigation, Volume2, VolumeX, AudioLines } from "lucide-react";
...
import DeliverySpeedTerms from './delivery-speed-terms';
import {bindDeliveryTiming} from '@/lib/game/delivery-timing';
import {deliveryElapsed,gateAvailability} from '@/lib/game/expedition-runtime';
...
import {useCockpitVoice} from '@/hooks/use-cockpit-voice';
...
export default function CargoGame(){
 ...
 const primeVoice=useCockpitVoice(voiceEnabled&&!muted&&aiReady===true&&mode==='flight'&&!paused&&flight.status==='flying',run,flight.warning);
 const deliveryClock=useRef(0),deliveryShortcuts=useRef({spawned:0,used:0});
 ...
 function resetExpedition(){
  const seed=crypto.getRandomValues(new Uint32Array(1))[0];deliveryClock.current=0;deliveryShortcuts.current={spawned:0,used:0};
 ...
 const previewContract=useMemo(()=>bindDeliveryTiming(contract,{cruise:effective.cruise??1,difficulty:campaign.difficulty}),[contract,effective.cruise,campaign.difficulty]);
 const gates=gateAvailability(flight);
  ...
  if(s.status!=='flying'){
   ...
   if(next!==campaignRef.current){
    ...
    if(s.status==='delivered'){
     ...
     const settled=resolveDelivery(expeditionRef.current,s.expedition?.contract??contractRef.current,{elapsed:deliveryElapsed(s),cargoIntegrity:s.cargo,damageTaken:s.expedition?.damageTaken??s.maxHull-(s.arrivalHull??s.hull),bonusReward:completed.reduce((sum,o)=>sum+o.definition.reward,0),objectivesCompleted:completed.map(o=>o.definition.uiText),objectivesFailed:failed.map(o=>o.definition.uiText)});
 ...
 async function launch(){
  if(voiceEnabled&&!muted)primeVoice();
  ...
  const started=beginAttempt(snapshot,{...template,contract:contractRef.current},true);if(!started)return;
  const vessel=effectiveHull(hull,started.campaign),condition=launchCondition(started.campaign,vessel);
  const timedContract=bindDeliveryTiming(contractRef.current,{cruise:vessel.cruise??1,difficulty:started.campaign.difficulty,usableBoosts:started.mission.events.filter(e=>e.kind==='portal').length});
  commitContract(timedContract);started.mission.contract=timedContract;started.mission.deliveryElapsed=deliveryClock.current;started.mission.deliveryShortcuts={...deliveryShortcuts.current};
 ...
 function depart(selected:Contract,kind:Upgrade|'continue'){
  ...
  nextContracts.current?.controller.abort();nextContracts.current=null;
  deliveryClock.current=0;deliveryShortcuts.current={spawned:0,used:0};
  setTransferFrom(campaignRef.current.stage);commitCampaign(next);commitContract(bindDeliveryTiming(selected,{cruise:effectiveHull(hull,next).cruise??1,difficulty:next.difficulty}));setDelivery(null);setContracts([]);setContractsLoading(false);setMode('transfer');setPaused(false);keys.current.clear();input.current={x:0,y:0,fire:false};setNotice('');
 }
