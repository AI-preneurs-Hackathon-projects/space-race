...
import DeliverySpeedTerms,{deliveryTime} from './delivery-speed-terms';
...
export default function ContractGate({result,contracts,loading,campaign,hull,onDepart}:{
  ...
  const ship=effectiveHull(hull,campaign),nextShip=effectiveHull(hull,{...campaign,cruiseUpgrades:Math.min(CRUISE_CAP,campaign.cruiseUpgrades+(upgrade==='cruise'?1:0))});
  const offers=contracts.map(contract=>bindDeliveryTiming(contract,{cruise:nextShip.cruise??1,difficulty:campaign.difficulty}));
  const chosen=offers.find(c=>c.id===selected);
  return <section ref={panel} className="result-panel contract-gate" role="dialog" aria-modal="true" aria-labelledby="gate-title" onKeyDown={event=>{
    ...
    <div className="delivery-ledger" aria-label="Delivery results">
      ...
      <div><span>Speed bonus</span><b>{credits(result.speedReward)} cr</b><small>{result.elapsed===null?'Clock unavailable':`${deliveryTime(result.elapsed)} active flight`}</small></div>
      <div><span>Objective bonus</span><b>{credits(result.bonusReward)} cr</b></div>
    ...
    {loading?<div className="contract-loading" role="status"><span className="contract-pulse"/> Receiving incoming contracts…</div>:<div className="contract-options" aria-label="Next contracts">
      {offers.map(contract=><button key={contract.id} className={`contract-option risk-${contract.risk} ${selected===contract.id?'selected':''}`} aria-pressed={selected===contract.id} onClick={()=>setSelected(contract.id)}>
        ...
        <DeliverySpeedTerms contract={contract}/>
        <strong>{credits(contract.reward)} <small>cr base</small></strong><span className="contract-destination">{contract.destination} · Sector {contract.sector}</span>
      </button>)}
    </div>}
    ...
    <small className="gate-footnote">Speed bonus requires successful cargo delivery. Late arrival keeps eligible base pay. Only active flight counts; terms reflect the selected loadout.</small>
  </section>;
}
