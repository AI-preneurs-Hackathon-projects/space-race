...
Ten named destination appearances share one fully wrapped sphere: Verdant, Selene, Ares, Nivalis, Aurelia, Pyra, Pelagia, Vesper, Nereid and Salar. Orientation, atmosphere and positions vary deterministically with the full stage number and remain stable on retries. The open-ended sequence cycles through the ten themes with numbered names. Nine worlds reuse their detailed source geography; the ocean world generates terrain from its stage seed. The fixed-radius sphere approaches with real route distance. Complete opaque 2048×1024 surface maps (1024×512 on mobile) combine licensed full-sphere planetary imagery with a distinct generated ocean-world terrain recipe. Relief/roughness and selected cloud/emission maps are loaded or generated once and disposed with the scene. Planet-only lighting keeps the day/night boundary readable. No destination Blender meshes are required; the TypeScript recipes and original vendored maps are reproducible source. See `assets/planets/README.md` for provenance and visual verification.

A completed delivery names its destination checkpoint. Choose an earned upgrade (or Continue when fully upgraded) to enter a separate, pausable terminal-gate transfer, then start the next stage with restored hull and cargo. This presentation never advances a disposed physics world or consumes either optional in-course jump. A standalone `RouteMap` below the hangar footer shows the completed/current/next neighborhood from the same destination metadata. There are no mid-stage checkpoints or persistent saves.
...
OpenAI remains optional. Hadrien's Site uses a separate server-side secret for the prelaunch director, with `gpt-4.1-mini` as the default model. No key from another project is used. `/api/ai-status` reports only whether a server credential exists; this does not prove generation works. `/api/ship` requests a constrained mirrored nine-section hull; `/api/mission` requests a prelaunch plan. The local stage generator retains the first 26 core waves, bounded coordinates, timing, difficulty budgets and both optional gates. A validated AI plan can select pirate encounters among later waves. Planning happens before launch; the active flight keeps its selected plan. There is no adaptive in-flight AI director. The existing sketch endpoint has not been expanded or revalidated in this activation.
