...
try{
 ...
 await page.evaluate(async()=>{
  ...
  let world;window.previewWorld=async(stage,mobile=false)=>{
   ...
   let opaque=true,poles=true,hash=2166136261;for(let i=0;i<data.length;i++){hash=Math.imul(hash^data[i],16777619);if(i%4===3&&data[i]!==255)opaque=false;}
   for(const row of [0,h-1])for(let x=1;x<w;x++)for(let c=0;c<4;c++)if(data[(row*w+x)*4+c]!==data[row*w*4+c])poles=false;
   const stats={stage,name:profile.name,kind:profile.kind,mobile,buildMs:performance.now()-start,width:w,height:h,opaque,poles,hash:hash>>>0,position:world.group.position.toArray(),textures:2+(profile.clouds?1:0)+(profile.kind==='volcanic'?1:0),roughness:!!globe.material.roughnessMap};
 ...
 for(let stage=1;stage<=10;stage++){
  const stats=await page.evaluate(stage=>window.previewWorld(stage),stage);if(!stats.opaque||!stats.poles)throw Error('Surface coverage: '+stage);report.push(stats);
  ...
  const mobile=await page.evaluate(stage=>window.previewWorld(stage,true),stage);if(!mobile.opaque||!mobile.poles)throw Error('Mobile coverage');report.push(mobile);await page.locator('#world-panel canvas').screenshot({path:`${dir}/stage-${stage}-mobile.png`});
 }
