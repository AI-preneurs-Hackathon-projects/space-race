...
report=json.loads((ROOT/'blender-report.json').read_text())
...
for row in report:
    ...
    for mesh in doc['meshes']:
        for p in mesh['primitives']:
            ...
            assert (np.einsum('ij,ij->i',cross,normals[indices.reshape((-1,3))].mean(axis=1))>0).all(), 'inverted normals'
    ...
    assert count==row['triangles'] and count<16000
    if row['name'] in ['iron-asteroid','ice-asteroid','volatile-rock','rogue-moon']:
        assert len(doc.get('images',[]))>=3
        assert all('bufferView' in im for im in doc['images'])
        assert all('TEXCOORD_0' in p['attributes'] for mesh in doc['meshes'] for p in mesh['primitives'])
        assert all('normalTexture' in m for m in doc['materials'])
    assert (ROOT/'blender'/(row['name']+'.blend')).is_file()
    print(f"{row['name']:20s} {count:4d} triangles  {len(raw):6d} bytes  {sum(len(m['primitives']) for m in doc['meshes'])} material batches  OK")
print(f"PASS: {len(report)} self-contained Blender GLBs, packed maps and editable sources")
